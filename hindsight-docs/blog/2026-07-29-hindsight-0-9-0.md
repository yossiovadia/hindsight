...
| **Reflect-based plugin, out-of-box** (bank built from empty, automatically; mean of 3 runs) | **0.72** | **−26%** |
| **Same, on banks matured by real sessions** (single run) | **0.55** | **−44%** |

All 33 tasks solved in every reflect run — 99/99 across the three out-of-box runs — injection verified on every task, and model cost *down* 35% ($0.79 → $0.51 per task), because one targeted synthesis replaces a lot of exploratory reading. On a stronger stack (Claude Code + Sonnet, our earlier five-run campaign with harness-built banks), the same mechanism cut corrections **58%**. The pattern we keep seeing: the better the agent, the more its remaining failures are exactly the non-guessable decisions memory carries.
...
Your data goes to *your* Hindsight server — self-hosted or cloud — with your keys.
